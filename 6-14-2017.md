...
The two definitions are essentially the same, but remember that, when the words on the left are the numbers from zero, *we can omit them!*

```
R = [1,2,3,0]
```

is exactly the same as the original `R`, and so it's even the same as our original `receives` function.
...
In a reversal, the signified become signifiers, and they signify the objects that were their signifiers. Put another way, words come to be characterized by their definitions.
...
Furthermore, if `S`' correspondence with `InitialTransition` is natural, then it is also natural that `T` should have a correspondingly natural relationship to some other, literally identical, dictionary. That dictionary, again
